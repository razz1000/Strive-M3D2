window.onload = () => {
    ...
    button5EventListener();
    countUniqueButtonEventListener();
...
/* From here and down I am pulling in the information from the Deezer API */
...
.then(songs => {
      ...
      for(let i = 0; i < songs.data.length; i++) {
          ...
          <img src="${songs.data[i].album.cover_big}" class="card-img-top" alt="...">
          <div class="card-body">
            ...
            <audio id="audio" src="${songs.data[i].preview}"></audio>
          </div>
        </div>
          `
...
/* END | pulling in the information from the Deezer API */


/* The Modal button  */

/* Button5 (modal) eventlistener */
let button5EventListener = () => {
  let button5 = document.querySelector(".button5")
  button5.addEventListener("click", displayAllTitlesInModal)
}

let displayAllTitlesInModal = (event) => {
  console.log(event)
  let modal1 = document.querySelector(".modal1");
  
  modal1.innerHTML = `
  <div
  class="modal fade modal1"
  id="exampleModal"
  tabindex="-1"
...
>
  ...
  const options = {
    ...
    .then(songs => {
      ...
      let div2 = document.querySelector(".modal-body")

          for(let i = 0; i < songs.data.length; i++) {
            let newCol = document.createElement("div")
            newCol.innerHTML = `<p>${songs.data[i].title}`
            newCol.classList.add("album-title-modal")
            div2.appendChild(newCol)

          }
})
}
/* END OF THE MODAL THAT IS PULLING THE QUEEN ALBUM TITLES */


/* Count unique button */
   let countUniqueButtonEventListener = () => {
     let button6 = document.querySelector(".button6")
     button6.addEventListener("click", countUniqueFunction)
   }



   /* This is the Count Unique function */
    let countUniqueFunction = (event) => {
      console.log(event)
      ...
      const options = {
        ...
        .then(songs => {
              ...
              let newUniqueArray = [];
              for(let i = 0; i < songs.data.length; i++) {
                if(!newUniqueArray.includes(songs.data[i].title)) {
                  newUniqueArray.push(songs.data[i].title)
                }
                }
                return console.log(newUniqueArray)

              })
             }


/* END:Count unique button */
